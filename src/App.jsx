...
function AIResult({ content, title = 'My Plan' }) {
  ...
  const handleDownload = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>${title} — PartyTripping</title>
          <style>
            body { font-family: Georgia, serif; max-width: 700px; margin: 40px auto; padding: 0 24px; color: #1a1a1a; line-height: 1.8; }
            h1 { font-size: 24px; margin-bottom: 6px; color: #071220; }
        ...
        <body>
          <h1>${title}</h1>
          <div class="meta">Generated by PartyTripping.com · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
          <pre>${content}</pre>
          <div class="footer">Free AI planning tools for friend groups · partytripping.com</div>
        </body>
      </html>
    `)
  ...
  return (
    <div style={{ marginTop: 20, background: 'rgba(0,201,177,0.06)', border: `1px solid ${C.borderTeal}`, borderRadius: 14, overflow: 'hidden' }}>
      {/* Header row with action buttons */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: `1px solid rgba(0,201,177,0.15)` }}>
        <div style={{ fontFamily: F.mono, fontSize: 10, color: C.teal, letterSpacing: '.1em', textTransform: 'uppercase' }}>✦ Your AI Plan</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleCopy} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 50, border: `1px solid ${C.borderTeal}`, background: copied ? `${C.teal}22` : 'transparent', color: copied ? C.teal : C.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: F.body, transition: 'all .2s' }}>
            {copied ? '✓ Copied' : '⎘ Copy'}
          </button>
          <button onClick={handleDownload} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 50, border: `1px solid rgba(184,241,66,0.3)`, background: 'transparent', color: C.lime, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: F.body, transition: 'all .2s' }}
            onMouseEnter={e => e.currentTarget.style.background = 'rgba(184,241,66,0.1)'}
      ...
      {/* Content */}
      <div style={{ padding: '20px 20px', fontSize: 14, color: C.white, lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{content}</div>
    </div>
  )
}
...
const TAG = 'grqk6540-20'
const amz = (query) => `https://www.amazon.com/s?k=${encodeURIComponent(query)}&tag=${TAG}`

function AmazonCTA({ title, links }) {
  return (
    <div style={{ marginTop: 20, padding: '18px 20px', background: 'rgba(255,140,66,0.06)', border: '1px solid rgba(255,140,66,0.2)', borderRadius: 14 }}>
      <div style={{ fontFamily: F.mono, fontSize: 10, color: C.orange, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>🛒 {title}</div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {links.map(([label, query]) => (
          <a key={label} href={amz(query)} target="_blank" rel="noopener noreferrer"
            style={{ fontSize: 12, fontWeight: 600, padding: '6px 14px', borderRadius: 50, background: 'rgba(255,140,66,0.12)', border: '1px solid rgba(255,140,66,0.25)', color: C.orange, textDecoration: 'none', transition: 'background .2s' }}>
            {label} →
          </a>
        ))}
      </div>
      <div style={{ fontSize: 10, color: C.muted, marginTop: 10, fontStyle: 'italic' }}>Amazon affiliate links — we may earn a small commission at no extra cost to you.</div>
    </div>
  )
}
...
Keep it warm, practical, and actionable. Use line breaks to organize clearly.`
  ...
  return (
    <div>
      ...
      <AIResult content={result} title={`${eventType} Plan`} />
      {result && <AmazonCTA title="Party supplies on Amazon" links={[['Party decorations', 'party decorations'], ['Tableware & plates', 'party tableware'], ['Balloons & banners', 'party balloons banners'], ['Photo booth props', 'photo booth props party'], ['Outdoor party lights', 'outdoor party string lights']]} />}
    </div>
  )
}
...
Keep it enthusiastic, practical, and group-focused.`
  ...
  return (
    <div>
      ...
      <AIResult content={result} title={`${destination} ${tripType} Plan`} />
      {result && <AmazonCTA title="Trip essentials on Amazon" links={[['Luggage & suitcases', 'carry on luggage'], ['Packing cubes', 'packing cubes travel'], ['Travel accessories', 'travel accessories'], ['Portable chargers', 'portable charger power bank'], ['Beach & pool gear', 'beach bag pool float']]} />}
    </div>
  )
}
...
Keep it practical and specific to this trip type. Format with clear category headers.`
  ...
  return (
    <div>
      ...
      <AIResult content={result} title={`${tripType} Packing List`} />
      {result && <AmazonCTA title="Shop your packing list on Amazon" links={[['Packing cubes', 'packing cubes travel organizer'], ['Toiletry bag', 'travel toiletry bag'], ['Luggage', 'carry on luggage spinner'], ['Travel pillow', 'travel neck pillow'], ['Sunscreen & skincare', 'travel size sunscreen']]} />}
    </div>
  )
}
...
Be practical and specific to ${city}.`
  ...
  return (
    <div>
      ...
      <AIResult content={result} title={`${city} Venue Ideas — ${partyType}`} />
      {result && <AmazonCTA title="Venue & event essentials on Amazon" links={[['Portable speaker', 'bluetooth speaker party outdoor'], ['LED party lights', 'LED party lights color changing'], ['Backdrop & decorations', 'party backdrop stand kit'], ['Serving platters', 'party serving platters trays'], ['Disposable cups & plates', 'party disposable plates cups']]} />}
    </div>
  )
}

// ─── TOOL 9: EVENTS FINDER (TICKETMASTER) ────────────────────────────────────
function EventsFinder() {
  ...
  const search = async () => {
    ...
    try {
      ...
      const url = `https://app.ticketmaster.com/discovery/v2/events.json?apikey=${key}&city=${encodeURIComponent(city)}&classificationId=${categoryMap[category]}&size=8&sort=date,asc`
      const res = await fetch(url)
      const data = await res.json()
      const items = data._embedded?.events || []
      setEvents(items)
      if (items.length === 0) setError(`No upcoming ${category} events found in ${city}. Try a nearby major city.`)
    } catch {
  ...
  return (
    <div>
      <div style={{ marginBottom: 14 }}><FieldLabel>City</FieldLabel>
        <input type="text" value={city} onChange={e => setCity(e.target.value)} onKeyDown={e => e.key === 'Enter' && search()} placeholder="e.g. Nashville, Miami, New York..." style={inputStyle} />
      </div>
      <div style={{ marginBottom: 14 }}><FieldLabel>Event Type</FieldLabel>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
          {[['music', '🎵', 'Music'], ['sports', '🏈', 'Sports'], ['arts', '🎭', 'Arts'], ['family', '👨‍👩‍👧', 'Family'], ['comedy', '😂', 'Comedy']].map(([val, emoji, label]) => (
            <div key={val} onClick={() => setCategory(val)} style={{ padding: '10px 8px', borderRadius: 10, border: `1px solid ${category === val ? C.teal : C.border}`, background: category === val ? `${C.teal}18` : C.navy3, cursor: 'pointer', textAlign: 'center' }}>
              <div style={{ fontSize: 20, marginBottom: 4 }}>{emoji}</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: category === val ? C.teal : C.muted }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
      <PrimaryBtn onClick={search} disabled={loading || !city.trim()}>
        {loading ? <><Spinner /> Finding events...</> : '🎟️ Find Events'}
      </PrimaryBtn>

      {error && <div style={{ marginTop: 16, padding: '14px 16px', background: 'rgba(255,92,77,0.08)', border: `1px solid ${C.borderCoral}`, borderRadius: 10, fontSize: 13, color: C.muted }}>{error}</div>}

      {events.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div style={{ fontFamily: F.mono, fontSize: 10, color: C.teal, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 14 }}>✦ {events.length} Events Found in {city}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {events.map((ev, i) => {
              const venue = ev._embedded?.venues?.[0]
              const date = ev.dates?.start?.localDate
              const time = ev.dates?.start?.localTime?.slice(0, 5)
              const img = ev.images?.find(img => img.ratio === '16_9' && img.width > 300)?.url
              const ticketUrl = ev.url
              const minPrice = ev.priceRanges?.[0]?.min
              const maxPrice = ev.priceRanges?.[0]?.max

              return (
                <div key={i} style={{ background: C.navy3, border: `1px solid ${C.border}`, borderRadius: 12, padding: '16px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  {img && <img src={img} alt={ev.name} style={{ width: 80, height: 54, objectFit: 'cover', borderRadius: 8, flexShrink: 0 }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ev.name}</div>
                    <div style={{ fontSize: 12, color: C.muted, marginBottom: 8 }}>
                      📅 {fmt(date)}{time ? ` · ${time}` : ''} &nbsp;·&nbsp; 📍 {venue?.name || 'Venue TBD'}
                      {minPrice && <span> &nbsp;·&nbsp; 💵 From ${Math.round(minPrice)}{maxPrice && maxPrice !== minPrice ? `–$${Math.round(maxPrice)}` : ''}</span>}
                    </div>
                    <a href={ticketUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 700, color: C.teal, background: `${C.teal}18`, border: `1px solid ${C.borderTeal}`, padding: '4px 12px', borderRadius: 50, display: 'inline-block' }}>
                      Get Tickets →
                    </a>
                  </div>
                </div>
          ...
          <div style={{ marginTop: 14, fontSize: 11, color: C.muted, fontStyle: 'italic' }}>Powered by Ticketmaster. Ticket links are affiliate links — PartyTripping may earn a commission.</div>
        </div>
      )}
    </div>
  )
...
// ─── TOOL 10: FLIGHT ESTIMATOR (AMADEUS) ─────────────────────────────────────
function FlightEstimator() {
  ...
  const search = async () => {
    ...
    try {
      ...
      const url = `https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=${origin.toUpperCase()}&destinationLocationCode=${destination.toUpperCase()}&departureDate=${date}&adults=1&max=5&currencyCode=USD`
      const res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } })
      const data = await res.json()
      const offers = data.data || []
      setResults(offers)
      if (offers.length === 0) setError('No flights found for this route and date. Try flexible dates or check airport codes.')
    } catch {
      setError('Could not load flights. Check your airport codes and try again.')
    }
    setLoading(false)
  }

  const totalCost = (pricePerPerson) => (parseFloat(pricePerPerson) * travelers).toFixed(0)

  return (
    <div>
      <div style={{ background: 'rgba(184,241,66,0.06)', border: `1px solid rgba(184,241,66,0.2)`, borderRadius: 10, padding: '10px 14px', marginBottom: 18, fontSize: 12, color: C.muted }}>
        💡 Use 3-letter airport codes (IATA): e.g. NYC = JFK or LGA, LA = LAX, Miami = MIA
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <div><FieldLabel>From (Airport Code)</FieldLabel>
          <input type="text" value={origin} onChange={e => setOrigin(e.target.value.toUpperCase())} placeholder="e.g. JFK" maxLength={3} style={{ ...inputStyle, textTransform: 'uppercase', letterSpacing: '.1em', fontFamily: F.mono }} />
        </div>
        <div><FieldLabel>To (Airport Code)</FieldLabel>
          <input type="text" value={destination} onChange={e => setDestination(e.target.value.toUpperCase())} placeholder="e.g. MIA" maxLength={3} style={{ ...inputStyle, textTransform: 'uppercase', letterSpacing: '.1em', fontFamily: F.mono }} />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <div><FieldLabel>Departure Date</FieldLabel>
          <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ ...inputStyle, colorScheme: 'dark' }} />
        </div>
        <div><FieldLabel>Group Size (for total cost)</FieldLabel>
          <input type="number" value={travelers} onChange={e => setTravelers(Math.max(1, Number(e.target.value)))} min={1} max={20} style={inputStyle} />
        </div>
      </div>
      <PrimaryBtn onClick={search} disabled={loading || !origin || !destination || !date}>
        {loading ? <><Spinner /> Searching flights...</> : '✈️ Find Flight Prices'}
      </PrimaryBtn>
      ...
      {results.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div style={{ fontFamily: F.mono, fontSize: 10, color: C.teal, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 14 }}>✦ Flight Options — {origin} → {destination}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {results.map((offer, i) => {
              const seg = offer.itineraries[0].segments[0]
              const dep = seg.departure.at?.slice(11, 16)
              const arr = seg.arrival.at?.slice(11, 16)
              const stops = offer.itineraries[0].segments.length - 1
              const price = parseFloat(offer.price.total)
              const groupTotal = (price * travelers).toFixed(0)
              const carrier = seg.carrierCode

              return (
                <div key={i} style={{ background: i === 0 ? `${C.teal}12` : C.navy3, border: `1px solid ${i === 0 ? C.borderTeal : C.border}`, borderRadius: 12, padding: '16px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  <div>
                    {i === 0 && <div style={{ fontFamily: F.mono, fontSize: 9, color: C.teal, letterSpacing: '.08em', marginBottom: 6 }}>LOWEST PRICE</div>}
                    <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>{dep} → {arr}</div>
                    <div style={{ fontSize: 12, color: C.muted }}>{carrier} · {stops === 0 ? 'Nonstop' : `${stops} stop${stops > 1 ? 's' : ''}`}</div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: F.mono, fontSize: 20, fontWeight: 600, color: i === 0 ? C.teal : C.white }}>${price.toFixed(0)}<span style={{ fontSize: 11, color: C.muted, fontWeight: 400 }}>/person</span></div>
                    <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>~${groupTotal} for {travelers} people</div>
                  </div>
                </div>
              )
            })}
          </div>
          <div style={{ marginTop: 14, padding: '12px 14px', background: 'rgba(0,201,177,0.06)', border: `1px solid ${C.borderTeal}`, borderRadius: 10, fontSize: 12, color: C.muted }}>
            💡 These are live estimates from Amadeus. Book directly with airlines or via Expedia, Google Flights, or Kayak for group discounts.
          </div>
          <div style={{ marginTop: 8, fontSize: 11, color: C.muted, fontStyle: 'italic' }}>Prices are per person, one-way. Powered by Amadeus Travel API.</div>
        </div>
      )}
    </div>
  )
...
const TRIP_TOOLS = [
  ...
  { id: 'events-finder', label: 'Events Finder', emoji: '🎟️', component: EventsFinder },
  { id: 'flight-estimator', label: 'Flight Estimator', emoji: '🛫', component: FlightEstimator },
  ...
  return (
    <div style={{ paddingTop: 80 }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '48px 48px 20px' }}>
        ...
        {/* Amazon CTA — contextual by article */}
        {article.cat === 'party' && article.id === 'how-to-plan-birthday-party' &&
          <AmazonCTA title="Birthday party supplies on Amazon" links={[['Birthday decorations', 'birthday party decorations'], ['Balloons & banners', 'birthday balloons banner set'], ['Tableware', 'birthday party tableware plates cups'], ['Party favors', 'birthday party favor bags'], ['Photo booth props', 'birthday photo booth props']]} />}
        {article.cat === 'party' && article.id === 'bachelorette-planning-guide' &&
          <AmazonCTA title="Bachelorette party essentials on Amazon" links={[['Bachelorette decorations', 'bachelorette party decorations'], ['Sashes & accessories', 'bride sash bachelorette accessories'], ['Games', 'bachelorette party games'], ['Tote bags', 'bachelorette party tote bags'], ['Matching shirts', 'bachelorette party shirts']]} />}
        {article.cat === 'party' && article.id === 'nye-party-checklist' &&
          <AmazonCTA title="NYE party supplies on Amazon" links={[['NYE decorations', 'new years eve party decorations'], ['Champagne flutes', 'champagne flutes set party'], ['Noisemakers & hats', 'new years eve noisemakers hats'], ['Countdown banner', 'happy new year banner'], ['Party favors', 'new years eve party favors']]} />}
        {article.cat === 'party' && article.id === 'surprise-party-tips' &&
          <AmazonCTA title="Surprise party supplies on Amazon" links={[['Surprise decorations', 'surprise party decorations kit'], ['Balloons', 'surprise party balloons'], ['Banners', 'surprise party banner'], ['Streamers', 'party streamers colorful'], ['Gift bags', 'party gift bags assorted']]} />}
        {article.cat === 'party' && article.id === 'graduation-party-guide' &&
          <AmazonCTA title="Graduation party supplies on Amazon" links={[['Graduation decorations', 'graduation party decorations'], ['Photo display', 'graduation photo display board'], ['Tableware', 'graduation party tableware'], ['Graduation banner', 'graduation banner 2026'], ['Party favors', 'graduation party favors']]} />}
        {article.cat === 'trip' && article.id === 'group-trip-no-drama' &&
          <AmazonCTA title="Group trip essentials on Amazon" links={[['Luggage sets', 'luggage set carry on'], ['Packing cubes', 'packing cubes travel set'], ['Portable speaker', 'waterproof bluetooth speaker'], ['Travel wallet', 'travel wallet rfid blocking'], ['Sunscreen', 'reef safe sunscreen SPF 50']]} />}
        {article.cat === 'trip' && article.id === 'weekend-getaway-ideas' &&
          <AmazonCTA title="Weekend getaway essentials on Amazon" links={[['Weekender bag', 'weekender bag overnight travel'], ['Portable charger', 'portable charger power bank'], ['Travel pillow', 'travel neck pillow memory foam'], ['Packing cubes', 'packing cubes small set'], ['Bluetooth speaker', 'mini bluetooth speaker waterproof']]} />}
        {article.cat === 'trip' && article.id === 'girls-trip-planner' &&
          <AmazonCTA title="Girls trip must-haves on Amazon" links={[['Matching shirts', 'girls trip shirts best friends'], ['Beach bag', 'large beach bag tote women'], ['Wine tumbler set', 'stemless wine tumbler set'], ['Passport holder', 'passport holder wallet women'], ['Inflatable pool float', 'inflatable pool float adult']]} />}
        {article.cat === 'group' && article.id === 'split-costs-group-trip' &&
          <AmazonCTA title="Travel money & organization on Amazon" links={[['Travel wallet', 'travel wallet organizer rfid'], ['Luggage tags', 'luggage tags personalized'], ['Money belt', 'money belt travel hidden'], ['Packing cubes', 'packing cubes travel organizer'], ['Portable scale', 'luggage scale portable digital']]} />}
      </div>
